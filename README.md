...
Генераторы можно использовать для ленивых итераций. Если вы знакомы с протоколом "Перебора", этот функционал появлся в 2015 году, тогда вы знаете что они тоже относятся к низкоуровневым конструкция. Эти протоколы говорят движку JS, что данный обект можно итерировать или использовать spread оператор для него. 
...
```
Преимущества redux saga, но надо помнить что все эти преимущества обоснованы функциями-генераторами. В очередной раз убеждаясь в том что они есть инструменты для интрументов.
