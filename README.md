...
Так например, стейт-менеджер redux-saga, использует для выполнения ассинхроных операций функции-генератора. Это очень удобно, и делает код визуально синхронным, как и async/await. Код который может выглядеть так
