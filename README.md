...
### Callback
Основным плюсом использовать [callback](https://developer.mozilla.org/ru/docs/Glossary/Callback_function) - это способ расширения функционала, элемент проектирования функций. Однако callback-функции, как и обычные функции, бывают как синхронные, так и ассинхронные. Синхронные callback-функции нужны для функциональной композиций. Ярким примером является функция pipe, которая используется очень часто.
```js
