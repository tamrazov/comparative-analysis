...
Итак, на последок попробую рассказать про еще один подход в проектировании систем. Это EventEmitter - это шаблон проектирования. О нем достаточно много написано, и это подход используется в реактивном программировании фреймворк RxJS. Однако я попробую разобрать его.

Как я писал выше, pull/push системы довольно частые конструкции в JavaScript. 

Концепция библиотеки React использует подход pull архитектуры. Это означает что когда React нужно вызвать функцию render, это вызовет рендер всех дочернир компонентов реакта. Это вернет новое предсталение которое потом реакт согласовывать с помощью React.reconcilation. Далее измениния будут преминены на DOM.

В этом и состоит pull концепция, в данном случае React, потому что именно он запрашивает новое состояние. 
